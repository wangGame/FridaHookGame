...
class DumpCommand(FridaIl2CppBridgeCommand[AssemblyDump | ClassDump, dict]):
    ...
    def parser(self) -> dict:
        return dict(
            help="performs a dump1 of the target application",
    ...
    def add_arguments(self, parser: argparse.ArgumentParser) -> None:
        parser.add_argument(
            ...
            type=Path,
            default=Path.cwd(),
            help="where to save the dump1 (defaults to current working dir)",
        )
        parser.add_argument(
            "--cs-output",
    ...
    def on_send(self, payload: AssemblyDump | ClassDump):
        if payload["type"] == "assembly":
            ...
            raise ValueError(f"Unknow dump1 type {payload}")
    ...
    def on_exit(self, payload: dict):
        ...
        if self.app.options.cs_output != "none":
            if self.app.options.cs_output != "stdout":
                self.app.update_status("Saving dump1...")
